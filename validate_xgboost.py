...
def processData(in_file_name, cust_dict):
	x_vars_list = []
	y_vars_list = []
	# rows_enum=enumerate(list(csv.DictReader(in_file_name)))
...
def processDataDf(in_file_name, cust_dict):
	x_vars_list = []
	y_vars_list = []
	for ii,row in tqdm(csv.DictReader(in_file_name)):
		# use only the four months as specified by breakfastpirate #
		if row['fecha_dato'] not in ['2015-05-28', '2015-06-28', '2016-05-28', '2016-06-28']:
			continue
...
def runXGB(train_X, train_y, seed_val=0):
	...
	return model
def predictXGB(model,valid_X):
    xgvalid=xgb.DMatrix(valid_X)
    
    pred=model.predict(xgvalid)
    print(pred)
    return pred
...
# print(cust_dict)
with open('cust_dict.json','w') as f:
...
print(train_X.shape)
print(train_y.shape)
print(x_vars_list[:10])
print(y_vars_list[:10])
j=0
...
model.load_model('recommendation.json')
print("Predicting..")
preds=predictXGB(model,test_X)
# del test_X
